<template>
  <div id="note-table">
    <el-table :data="bookings" style="width: 100%">
        ...
        <template slot-scope="props">
          ...
          <h5>Thông tin booking</h5>
            <div v-for="(item, index) in props.row.detail.services" :key="index">
              <span class="mr-2">{{item.service_name}}:</span>
      ...
      <el-table-column label="Tên khách hàng" sortable>
        <template slot-scope="scope">
          <div class="name-booking">{{scope.row.customer.name}}</div>
        </template>
      </el-table-column>
      <el-table-column label="Số điện thoại" sortable>
        <template slot-scope="scope">
          <div class="name-booking">{{scope.row.customer.phone}}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="Thợ" sortable>
        <template slot-scope="scope">
          <div class="name-booking">{{scope.row.barber.name}} - {{scope.row.barber.phone}}</div>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('booking', ['fetchAdminBookings', 'removeBooking', 'updateBookingStatus']),
